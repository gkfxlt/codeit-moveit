...
namespace codeit::system {
	...
	public:
		...
		auto setRtPreCallback(PreCallback pre_callback)->void;
		auto setRtPostCallback(PostCallback post_callback)->void;
		...
		auto nrtMotionState()->std::uint64_t;//非实时模型的状态
		auto setNrtMotionState(std::uint64_t)->void;//非实时模型的状态
