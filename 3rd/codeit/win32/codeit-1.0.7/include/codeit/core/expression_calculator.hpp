#ifndef CODEIT_CORE_EXPRESSION_CALCULATOR_H_
#define CODEIT_CORE_EXPRESSION_CALCULATOR_H_
...
namespace codeit::core
{
	...
	public:
		...
		///***新增
		auto deleteVariable(std::string_view var)->void;
		auto revalueVariable(std::string_view var, std::string_view type, const std::any& value)->void;

		virtual ~Calculator();
		explicit Calculator(const std::string& name = "calculator");
		CODEIT_DECLARE_BIG_FOUR(Calculator);
		//ARIS_REGISTER_TYPE(Calculator)
	private:
		struct Imp;
		core::ImpPtr<Imp> imp_;
	};
	class LanguageParser : public core::Object
	{
	public:
		...
		CODEIT_REGISTER_TYPE(LanguageParser);
		CODEIT_DECLARE_BIG_FOUR(LanguageParser);
	...
	auto defaultUserDataType(core::Calculator& cal)->void;
}
