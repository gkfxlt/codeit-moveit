...
namespace codeit::function {
	...
	class OpenToqOffset : public BasisFunc
	{
	public:
		auto virtual prepareNrt(BasisFunc& plan_next, int)->void;
		explicit OpenToqOffset(const std::string& name = "OpenToqOffset");
		virtual ~OpenToqOffset();
		CODEIT_REGISTER_TYPE(OpenToqOffset);
		CODEIT_DECLARE_BIG_FOUR(OpenToqOffset);
	};


	class OpenVelOffset : public BasisFunc
	{
	public:
		auto virtual prepareNrt(BasisFunc& plan_next, int)->void;
		explicit OpenVelOffset(const std::string& name = "OpenVelOffset");
		virtual ~OpenVelOffset();
		CODEIT_REGISTER_TYPE(OpenVelOffset);
		CODEIT_DECLARE_BIG_FOUR(OpenVelOffset);
	};
