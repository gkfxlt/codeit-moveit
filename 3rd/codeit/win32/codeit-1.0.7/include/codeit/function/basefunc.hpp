...
namespace codeit::function {
	...
	public:
		...
		// state machine of nrt//
		enum NrtMotionState : std::uint64_t
		{
			NRTOPERATION = 0x01ULL << 0,
			NRTSTOPPING = 0x01ULL << 1,
			NRTERRORSTOP = 0x01ULL << 2,
			NRTEMERGENCYSTOP = 0x01ULL << 3,
			NRTCLEARFUNCPOOL = 0x01ULL << 4,
		};
