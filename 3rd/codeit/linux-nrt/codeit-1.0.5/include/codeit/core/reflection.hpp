#ifndef CODEIT_CORE_REFLECTION_H_
#define CODEIT_CORE_REFLECTION_H_
...
namespace codeit::core
{
	...
	public:
			...
			:Instance(&typeid(t), dynamic_cast<void*>(&const_cast<std::decay_t<T>&>(t)))
		{
			...
			if (type() == nullptr) THROW_FILE_LINE("Unrecognized type : " + std::string(typeid(t).name()));
		}
	...
	public:
		...
		{
			if (!std::is_base_of_v<FatherType, Class_Type>)std::cout << (std::string("failed to inherit \"") + std::string(typeid(FatherType).name()) + "\" for \"" + std::string(typeid(Class_Type).name()) + "\"") << std::endl;
			if (!std::is_base_of_v<FatherType, Class_Type>)THROW_FILE_LINE(std::string("failed to inherit \"") + std::string(typeid(FatherType).name()) + "\" for \"" + std::string(typeid(Class_Type).name()) + "\"");
