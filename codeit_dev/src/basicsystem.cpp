...
//*************************创建控制器***********************//////
auto createVrepControllerPr2() -> std::unique_ptr<
    codeit::controller::Controller> /*函数返回的是一个类指针，指针指向Controller,controller的类型是智能指针std::unique_ptr*/
{
  std::unique_ptr<codeit::controller::Controller> controller(new codeit::controller::VrepController);

  std::string names[] = { "r_shoulder_pan_joint", "r_shoulder_lift_joint", "r_upper_arm_roll_joint",
  ...
  double zero_offset[14]{ 0, 0 * PI / 2, 0, 0 * PI / 2, 0, 0 };
  double pos_offset[14]{ 0, 0, 0, 0, 0, 0 };
  double pos_factor[14]{ 360 / 2 / PI, 360 / 2 / PI, 360 / 2 / PI, 360 / 2 / PI, 360 / 2 / PI, 360 / 2 / PI };
  double max_pos[14]{
    170.0 / 360 * 2 * PI, 170.0 / 360 * 4 * PI, 170.0 / 360 * 2 * PI,
    170.0 / 360 * 4 * PI, 117.0 / 360 * 2 * PI, 360.0 / 360 * 2 * PI,
  };
  double min_pos[14]{ -170.0 / 360 * 2 * PI, -170.0 / 360 * 4 * PI, -170.0 / 360 * 2 * PI,
                      -170.0 / 360 * 4 * PI, -117.0 / 360 * 2 * PI, -360.0 / 360 * 2 * PI };
  double max_vel[14]{
    310.0 / 360 * 2 * PI, 240.0 / 360 * 2 * PI, 310.0 / 360 * 2 * PI,
    250.0 / 360 * 2 * PI, 295.0 / 360 * 2 * PI, 500.0 / 360 * 2 * PI,
  };
  double max_acc[14]{
    15000.0 / 360 * 2 * PI, 15000.0 / 360 * 2 * PI, 15000.0 / 360 * 2 * PI,
    17500.0 / 360 * 2 * PI, 15000.0 / 360 * 2 * PI, 25000.0 / 360 * 2 * PI,
  };
  double tor_const[14]{ 0.283 * 4808, 0.283 * 4808, 0.276 * 2546, 0.226 * 1556, 0.219 * 849, 0.219 * 849 };

  for (Size i = 0; i < 14; ++i)
  {
    zero_offset[i] = 0;
    pos_offset[i] = 0;
    pos_factor[i] = 360.0 / 2 / PI;
    max_pos[i] = 360.0 / 360 * 2 * PI;
    min_pos[i] = -360.0 / 360 * 2 * PI;
    max_vel[i] = 500.0 / 360 * 2 * PI;
    max_acc[i] = 15000.0 / 360 * 2 * PI;
    tor_const[i] = 0.219 * 849;

    //" zero_offset=\"" + std::to_string(zero_offset[i]) + "\""
    //+std::to_string(min_pos[i]) +
    std::string joint_handle = names[i];
    std::string xml_str = "<VrepMotor phy_id=\"" + std::to_string(i) + "\" handle=\"" + joint_handle +
                          "\""
                          " min_pos=\"" +
                          std::to_string(min_pos[i]) + "\" max_pos=\"" + std::to_string(max_pos[i]) + "\" max_vel=\"" +
                          std::to_string(max_vel[i]) + "\" min_vel=\"" + std::to_string(-max_vel[i]) +
                          "\" zero_offset=\"" + std::to_string(zero_offset[i]) +
                          "\""
                          " max_acc=\"" +
                          std::to_string(max_acc[i]) + "\" min_acc=\"" + std::to_string(-max_acc[i]) +
                          "\" max_pos_following_error=\"0.1\" max_vel_following_error=\"0.5\""
                          " tor_const=\"" +
                          std::to_string(tor_const[i]) +
                          "\""
                          " home_pos=\"0\" pos_factor=\"" +
                          std::to_string(pos_factor[i]) + "\" pos_offset=\"" + std::to_string(pos_offset[i]) +
                          "\">"
...
auto createVrepController() -> std::unique_ptr<
    codeit::controller::Controller> /*函数返回的是一个类指针，指针指向Controller,controller的类型是智能指针std::unique_ptr*/
{
  std::unique_ptr<codeit::controller::Controller> controller(new codeit::controller::VrepController);

  std::string names[] = { "shoulder_pan_joint", "shoulder_lift_joint", "elbow_joint",
                          "wrist_1_joint",      "wrist_2_joint",       "wrist_3_joint" };
  for (Size i = 0; i < 6; ++i)
  {
    double zero_offset[6]{ 0, 0 * PI / 180, 0 * PI / 180, 0 * PI / 2, 0, 0 };
    double pos_offset[6]{ 0, 0, 0, 0, 0, 0 };
    double pos_factor[6]{ 360 / 2 / PI, 360 / 2 / PI, 360 / 2 / PI, 360 / 2 / PI, 360 / 2 / PI, 360 / 2 / PI };
    double max_pos[6]{
      360.0 / 360 * 2 * PI, 360.0 / 360 * 4 * PI, 360.0 / 360 * 2 * PI,
      360.0 / 360 * 4 * PI, 360.0 / 360 * 2 * PI, 360.0 / 360 * 2 * PI,
    };
    double min_pos[6]{ -360.0 / 360 * 2 * PI, -360.0 / 360 * 4 * PI, -360.0 / 360 * 2 * PI,
                       -360.0 / 360 * 4 * PI, -360.0 / 360 * 2 * PI, -360.0 / 360 * 2 * PI };
    double max_vel[6]{
      310.0 / 360 * 2 * PI, 240.0 / 360 * 2 * PI, 310.0 / 360 * 2 * PI,
      250.0 / 360 * 2 * PI, 295.0 / 360 * 2 * PI, 500.0 / 360 * 2 * PI,
    ...
    double tor_const[6]{ 0.283 * 4808, 0.283 * 4808, 0.276 * 2546, 0.226 * 1556, 0.219 * 849, 0.219 * 849 };
...
auto createEcatController() -> std::unique_ptr<
    codeit::controller::Controller> /*函数返回的是一个类指针，指针指向Controller,controller的类型是智能指针std::unique_ptr*/
{
  std::unique_ptr<codeit::controller::Controller> controller(
      new codeit::controller::EthercatController); /*创建std::unique_ptr实例*/
  controller->slavePool().clear();                 //清除slavePool中的元素，后面重新添加
  for (Size i = 0; i < 1; ++i)
  {
#ifdef UNIX
    double pos_offset[6]{ 1.4327,           2.41069874505508,   0.762016940604939,
                          4.69692333813102, 0.0360485485152105, 1.43257027673246 };
#endif
#ifdef WIN32
    double pos_offset[6]{ 0, 0, 0, 0, 0, 0 };
#endif
    double pos_factor[6]{ 1048576.0 / 2 / PI,  1048576.0 / 2 / PI, 1048576.0 / 2 / PI,
                          -1048576.0 / 2 / PI, -524288.0 / 2 / PI, 524288.0 / 2 / PI };
    double max_pos[6]{ 180.0 / 360 * 2 * PI, 120.0 / 360 * 2 * PI, 60.0 / 360 * 2 * PI,
                       180.0 / 360 * 2 * PI, 120.0 / 360 * 2 * PI, 180.0 / 360 * 2 * PI };
    double min_pos[6]{ -180.0 / 360 * 2 * PI, -120.0 / 360 * 2 * PI, -240.0 / 360 * 2 * PI,
                       -180.0 / 360 * 2 * PI, -120.0 / 360 * 2 * PI, -180.0 / 360 * 2 * PI };
    double max_vel[6]{ 0.4, 0.4, 0.4, 0.4, 0.6, 0.6 };
    double max_acc[6]{ //                3.3, 3.3*2, 3.3, 3.3, 7.5, 7.5
                       //                10, 10, 10, 10, 25, 25
                       20, 20, 20, 20, 50, 50
    };

    std::string xml_str = "<EthercatMotor phy_id=\"" + std::to_string(i) +
                          "\" product_code=\"0x201\""
...
auto createZeroErrEcatController() -> std::unique_ptr<
    codeit::controller::Controller> /*函数返回的是一个类指针，指针指向Controller,controller的类型是智能指针std::unique_ptr*/
{
  std::unique_ptr<codeit::controller::Controller> controller(
      new codeit::controller::EthercatController); /*创建std::unique_ptr实例*/
  ...
  for (Size i = 0; i < 1; ++i)
  {
    ...
    std::string xml_str =
        "<EthercatMotor phy_id=\"" + std::to_string(i) +
        "\" product_code=\"0x29252\""
...
auto createSocketController(const NumList* numList, std::string name, std::string ip, std::string port,
                            SocketMaster::TYPE type, Size nrt_id) -> std::unique_ptr<codeit::controller::NrtController>
{
  static const NumList socket_num_list = { 0 };
  numList = numList ? numList : &socket_num_list;

  std::unique_ptr<codeit::controller::NrtController> controller(
      new codeit::controller::SocketController(name, ip, port, type, nrt_id));

  int begin_index = 0;
  for (Size i = 0; i < numList->di_num; ++i)
  {
    std::string xml_str = "<SocketDI phy_id=\"" + std::to_string(begin_index) +
                          "\"is_virtual=\"false\">"
                          "</SocketDI>";
  ...
  {
    std::string xml_str = "<SocketDO phy_id=\"" + std::to_string(begin_index) +
                          "\"is_virtual=\"false\">"
                          "</SocketDO>";
  ...
  {
    std::string xml_str = "<SocketAI phy_id=\"" + std::to_string(begin_index) + "\"is_virtual=\"false\" resolution=\"" +
                          std::to_string(res[i]) +
                          "\">"
                          "</SocketAI>";
  ...
  {
    std::string xml_str = "<SocketAO phy_id=\"" + std::to_string(begin_index) + "\"is_virtual=\"false\" resolution=\"" +
                          std::to_string(res[i]) +
                          "\">"
                          "</SocketAO>";
...
#endif
#ifdef UNIX
    double pos_offset[6]{ 0.0345045068966465,  0.151295566371175,  -0.181133422007823,
                          0.00569660673541914, 0.0119907348546894, 0.0908806917782888 };
#endif
    double pos_factor[6]{ 131072.0 * 129.6 / 2 / PI, -131072.0 * 100 / 2 / PI, 131072.0 * 101 / 2 / PI,
                          131072.0 * 81.6 / 2 / PI,  131072.0 * 81 / 2 / PI,   131072.0 * 51 / 2 / PI };
    double max_pos[6]{ 170.0 / 360 * 2 * PI, 40.0 / 360 * 2 * PI,  150.0 / 360 * 2 * PI,
                       180.0 / 360 * 2 * PI, 125.0 / 360 * 2 * PI, 360.0 / 360 * 2 * PI };
    double min_pos[6]{ -170.0 / 360 * 2 * PI, -165.0 / 360 * 2 * PI, -125.0 / 360 * 2 * PI,
                       -180.0 / 360 * 2 * PI, -125.0 / 360 * 2 * PI, -360.0 / 360 * 2 * PI };
    double max_vel[6]{ 230.0 / 360 * 2 * PI, 300.0 / 360 * 2 * PI, 300.0 / 360 * 2 * PI,
                       300.0 / 360 * 2 * PI, 375.0 / 360 * 2 * PI, 600.0 / 360 * 2 * PI };
    double max_acc[6]{ 1150.0 / 360 * 2 * PI, 1500.0 / 360 * 2 * PI, 1500.0 / 360 * 2 * PI,
                       1500.0 / 360 * 2 * PI, 1875.0 / 360 * 2 * PI, 3000.0 / 360 * 2 * PI };

    std::string xml_str = "<SocketMotor phy_id=\"" + std::to_string(begin_index) +
                          "\" "
                          " min_pos=\"" +
                          std::to_string(min_pos[i]) + "\" max_pos=\"" + std::to_string(max_pos[i]) + "\" max_vel=\"" +
                          std::to_string(max_vel[i]) + "\" min_vel=\"" + std::to_string(-max_vel[i]) +
  ...
  {
    std::string xml_str = "<SocketSubSystem phy_id=\"" + std::to_string(begin_index) +
                          "\"is_virtual=\"false\">"
                          "</SocketSubSystem>";
...
auto createComController(const NumList* numList, const std::string& name, const core::SerialPort::ComOptions& options,
                         Size pack_size, Size nrt_id) -> std::unique_ptr<codeit::controller::NrtController>
{
  static const NumList com_num_list = { 0 };
  numList = numList ? numList : &com_num_list;

  std::unique_ptr<codeit::controller::NrtController> controller(
      new codeit::controller::ComController(name, options, pack_size, nrt_id));

  int begin_index = 0;
  for (Size i = 0; i < numList->di_num; ++i)
  {
    std::string xml_str = "<ComDI phy_id=\"" + std::to_string(begin_index) +
                          "\"is_virtual=\"false\">"
                          "</ComDI>";
  ...
  for (Size i = 0; i < numList->do_num; ++i)
  {
    std::string xml_str = "<ComDO phy_id=\"" + std::to_string(begin_index) +
                          "\"is_virtual=\"false\">"
                          "</ComDO>";
  ...
  for (Size i = 0; i < numList->ai_num; ++i)
  {
    std::string xml_str = "<ComAI phy_id=\"" + std::to_string(begin_index) + "\"is_virtual=\"false\" resolution=\"" +
                          std::to_string(res[i]) +
                          "\">"
                          "</ComAI>";
  ...
  for (Size i = 0; i < numList->ao_num; ++i)
  {
    std::string xml_str = "<ComAO phy_id=\"" + std::to_string(begin_index) + "\"is_virtual=\"false\" resolution=\"" +
                          std::to_string(res[i]) +
                          "\">"
                          "</ComAO>";
  ...
  for (Size i = 0; i < numList->motor_num; ++i)
  {
    ...
    std::string xml_str = "<ComMotor phy_id=\"" + std::to_string(begin_index) +
                          "\" "
                          " min_pos=\"" +
                          std::to_string(min_pos[i]) + "\" max_pos=\"" + std::to_string(max_pos[i]) + "\" max_vel=\"" +
                          std::to_string(max_vel[i]) + "\" min_vel=\"" + std::to_string(-max_vel[i]) +
  ...
  for (Size i = 0; i < numList->subsys_num; ++i)
  {
    std::string xml_str = "<ComSubSystem phy_id=\"" + std::to_string(begin_index) +
                          "\"is_virtual=\"false\">"
                          "</ComSubSystem>";
...
{
  std::unique_ptr<core::ObjectPool<codeit::controller::NrtController>> nrtControllerPool(
      new core::ObjectPool<codeit::controller::NrtController>);
...
auto createModelPool() -> std::unique_ptr<core::ObjectPool<codeit::model::Model>>
{
  ...
  auto moveit_planner = MoveitPlanner::getInstance();
...
{
  std::unique_ptr<core::ObjectPool<codeit::system::ErrorInfo>> errorinfoPool(
      new core::ObjectPool<codeit::system::ErrorInfo>);
  ...
  errorinfoPool->add<codeit::system::ErrorInfo>("master name not exist", code, errLevel, "主站不存在.",
                                                "master name not exist");
...
auto createFuncRoot() -> std::unique_ptr<codeit::function::FuncRoot>
{
  ...
  cmd_root->funcPool().add<codeit::function::MoveJS>();
  cmd_root->funcPool().add<codeit::function::MoveIt>();
  cmd_root->funcPool().add<codeit::function::ObsM>();
  cmd_root->funcPool().add<codeit::function::CheckJS>();
  //  cmd_root->funcPool().add<codeit::function::Perception>();
