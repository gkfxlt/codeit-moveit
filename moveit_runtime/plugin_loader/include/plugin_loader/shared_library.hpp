...
public:
  ...
  // void load(const std::string& path); fan mask todo:
...
inline bool SharedLibrary::isLoaded() const
{
  return _handle != 0;
}
...
inline const std::string& SharedLibrary::getPath() const
{
  return _path;
}

inline std::string SharedLibrary::prefix()
{
...
inline std::string SharedLibrary::suffix()
{
#if CL_OS == CL_OS_MAC_OS_X
  return ".dylib";
#elif CL_OS == CL_OS_HPUX
  return ".sl";
#elif CL_OS == CL_OS_WINDOWS_NT
  return ".dll";
...
inline std::string SharedLibrary::suffix_d()
{
  return 'd' + suffix();
}
